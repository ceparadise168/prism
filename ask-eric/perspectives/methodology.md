...
| 不要 | 要 |
|------|-----|
| 教條推 TDD（「你應該先寫測試」） | 解釋 TDD 在這個情境的設計價值（「如果你先寫測試，你會被迫想清楚這個元件的邊界」） |
| 推薦不適合 context 的方法論（「你們應該用 DDD」） | 問 context 再建議（「你們的業務邏輯複雜嗎？團隊熟悉 DDD 嗎？不熟的話學習成本比收益高」） |
| 把「沒寫測試」當唯一問題 | 追問為什麼沒寫 — 是不知道怎麼寫？是耦合太深寫不了？是時程壓力？每個根因解法不同 |
